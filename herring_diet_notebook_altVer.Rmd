...
herring.environ <- herring.basin.data.mod %>% 
  ...
  dplyr::rename(`SOG herring` = SOG.Herring, `Cherry Point herring` = cherry_point, `Other herring` = other_herring, Chinook = chinook.ab, Chum = chum.ab, Coho = coho.ab, Pink = pink.ab, Sockeye = sockeye.ab, Steelhead = steelhead.ab, `Southern Resident orcas`= Orcas, `Harbour seals` = Seals.Nelson, SST = RR.SSTOut, `Chl a` = Chla.JDF, `Fraser river flow`=FraserFlow.Out)
...
herring.pc.final <- herring_gam_pc_m4
...
herring.viz.term.1 <- plot(sm(herring.viz, 5)) +
  l_ciPoly() +
  l_fitLine(colour = "black") + 
  theme_classic() +
  labs(tag = "A", x="Gaussian quantiles", y = "Effects", title="s(Predator group):basin",
  subtitle="San Juan Islands")

herring.viz.term.2 <- plot(sm(herring.viz, 13)) +
    l_ciPoly() +
  l_fitLine(colour = "black") + 
  ...
  theme_classic() +
  labs(tag = "B") +
  labs(x="Predator size", y = "s(Predator size):basin San Juan Is.") 
 # ggtitle("s(Predator size):basin San Juan Islands")

herring.viz.term.3 <- plot(sm(herring.viz, 21)) +
    l_ciPoly() +
  l_fitLine(colour = "black") + 
  ...
  theme_classic() +
  labs(tag = "C") +
  labs(x="Year", y = "s(year: basin San Juan Is.")
  #ggtitle("s(year: basin San Juan Archipelago")
...
model.names <- paste0("model_",1:10)
no.fits <- 1:10
...
herring.m.final <- herring_gam_vars_m7
...
herring.viz.term.1 <- plot(sm(herring.viz, 2)) +
  l_ciPoly() +
  l_fitLine(colour = "black") + 
  theme_classic() +
  labs(tag = "A", x="Gaussian quantiles", y = "Effects", title = "s(basin): year")

herring.viz.term.2 <- plot(sm(herring.viz, 6)) +
  l_ciPoly() +
  l_fitLine(colour = "black") + 
  theme_classic() +
  labs(tag = "B", x="Chinook abundance", y = "s(Chinook abundance)")

herring.viz.term.3 <- plot(sm(herring.viz, 14)) +
  l_ciPoly() +
  l_fitLine(colour = "black") + 
  theme_classic() +
  labs(tag = "C", x="NPGO", y = "s(NPGO)")

herring.viz.term.4 <- plot(pterm(herring.viz, 3)) +
  l_ciPoly() +
  l_fitLine(colour = "black") + 
  theme_classic() +
  labs(tag = "D", x="Other herring", y = "f(Other herring)")

herring.viz.term.5 <- plot(pterm(herring.viz, 4)) +
   l_ciPoly() +
   l_fitLine(colour = "black") + 
   labs(tag = "E", x="Chum abundance", y = "f(Chum abundance)")+
  theme_classic()

herring.viz.term.6 <- plot(pterm(herring.viz, 6)) +
   l_ciPoly() +
   l_fitLine(colour = "black") + 
   labs(tag = "F", x="Sockeye abundance", y = "f(Sockeye abundance)")+
  theme_classic()

herring.viz.term.7 <- plot(pterm(herring.viz, 7)) +
   l_ciPoly() +
   l_fitLine(colour = "black") + 
   labs(tag = "F", x="Pink abundance", y = "f(Pink abundance)")+
  theme_classic()

herring.viz.term.8 <- plot(pterm(herring.viz, 8)) +
   l_ciPoly() +
   l_fitLine(colour = "black") + 
   labs(tag = "F", x="Chl a", y = "f(Chl a)")+
  theme_classic()
...
grid.sample <- gridPrint(herring.viz.term.1, herring.viz.term.2, herring.viz.term.3,
                         ...
                         herring.viz.term.7, herring.viz.term.8, ncol=3, nrow=3)


ggsave("vars_gam_plot.png", grid.sample, device="png",width=8,height=7, dpi=350)
...
list.fits <- list(herring_gam_co_vars_m1,herring_gam_co_vars_m2,herring_gam_co_vars_m3,herring_gam_co_vars_m4,herring_gam_co_vars_m5,herring_gam_co_vars_m6,herring_gam_co_vars_m7,herring_gam_co_vars_m8, herring_gam_co_vars_m9, herring_gam_co_vars_m10)
...
herring.m.final <- herring_gam_co_vars_m4
...
herring.viz.term.2 <- plot(sm(herring.viz, 13)) +
  ...
  l_fitLine(colour = "black") + 
  theme_classic() +
  labs(tag = "B", x="Year", y = "s(year:basin San Juan Islands)")

herring.viz.term.3 <- plot(sm(herring.viz, 21)) +
  l_ciPoly() +
  l_fitLine(colour = "black") + 
  theme_classic() +
  labs(tag = "C", x="Predator size", y = "s(predator size:basin San Juan Islands)")

herring.viz.term.4 <- plot(pterm(herring.viz, 2)) +
  l_fitLine(colour = "black") + 
#  l_points(colour = "darkblue", size=1) + 
  l_rug(alpha = 0.3) +
  labs(tag = "D", x="Year", y = "f(Year)")+
  theme_classic()


grid.sample <- gridPrint(herring.viz.term.1, herring.viz.term.2, herring.viz.term.3,
                         herring.viz.term.4, ncol=2, nrow=2)
